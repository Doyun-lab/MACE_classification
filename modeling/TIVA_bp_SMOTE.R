...
setwd("/Users/kwondoyun/Downloads/tiva_bp")
...
## XGBoost
set.seed(234)
conf_tiva_bp_xgb <- list()
for (i in 1:10){
  train <- tiva_bp_train[[i]]
  test <- tiva_bp_test[[i]]
  ...
  conf_tiva_bp_xgb[[i]] <- conf_mat$table
}

## Random Forest
set.seed(234)
conf_tiva_bp_rf <- list()
for (i in 1:10){
  train <- tiva_bp_train[[i]]
  test <- tiva_bp_test[[i]]
  ...
  conf_tiva_bp_rf[[i]] <- conf_mat$table
  cat("Fold", i, "complete \n", sep = " ")
}

## DNN
set.seed(234)
conf_tiva_bp_dnn <- list()
for (i in 1:10){
  train <- tiva_bp_train[[i]]
  test <- tiva_bp_test[[i]]
  ...
  conf_tiva_bp_dnn[[i]] <- conf_mat$table
  cat("Fold", i, "complete \n", sep = " ")
}

## Confusion Matrix
confusion_matrix(conf_tiva_bp_xgb)
confusion_matrix(conf_tiva_bp_rf)
confusion_matrix(conf_tiva_bp_dnn)
