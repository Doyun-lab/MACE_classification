...
setwd("/Users/kwondoyun/Downloads/tiva_red")
...
for (f in fls_test){
  ...
  tiva_red_test[[i]] <- temp
  i <- i + 1
}

## XGBoost
set.seed(234)
conf_tiva_red_xgb <- list()
for (i in 1:10){
  train <- tiva_red_train[[i]]
  test <- tiva_red_test[[i]]
  ...
  conf_tiva_red_xgb[[i]] <- conf_mat$table
}

## Random Forest
set.seed(234)
conf_tiva_red_rf <- list()
for (i in 1:10){
  train <- tiva_red_train[[i]]
  test <- tiva_red_test[[i]]
  ...
  conf_tiva_red_rf[[i]] <- conf_mat$table
  cat("Fold", i, "complete \n", sep = " ")
}

## DNN
set.seed(234)
conf_tiva_red_dnn <- list()
for (i in 1:10){
  train <- tiva_red_train[[i]]
  test <- tiva_red_test[[i]]
  ...
  conf_tiva_red_dnn[[i]] <- conf_mat$table
  cat("Fold", i, "complete \n", sep = " ")
}

## Confusion Matrix
confusion_matrix(conf_tiva_red_xgb)
confusion_matrix(conf_tiva_red_rf)
confusion_matrix(conf_tiva_red_dnn)
