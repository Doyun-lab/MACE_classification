...
setwd("/Users/kwondoyun/Downloads/vol_red")
...
## XGBoost
set.seed(234)
conf_vol_red_xgb <- list()
for (i in 1:10){
  train <- vol_red_train[[i]]
  test <- vol_red_test[[i]]
  ...
  conf_vol_red_xgb[[i]] <- conf_mat$table
}

## Random Forest
set.seed(234)
conf_vol_red_rf <- list()
for (i in 1:10){
  train <- vol_red_train[[i]]
  test <- vol_red_test[[i]]
  ...
  conf_vol_red_rf[[i]] <- conf_mat$table
  cat("Fold", i, "complete \n", sep = " ")
}

## DNN
set.seed(234)
conf_vol_red_dnn <- list()
for (i in 1:10){
  train <- vol_red_train[[i]]
  test <- vol_red_test[[i]]
  ...
  conf_vol_red_dnn[[i]] <- conf_mat$table
  cat("Fold", i, "complete \n", sep = " ")
}

## Confusion Matrix
confusion_matrix(conf_vol_red_xgb)
confusion_matrix(conf_vol_red_rf)
confusion_matrix(conf_vol_red_dnn)
