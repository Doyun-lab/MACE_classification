...
setwd("/Users/kwondoyun/Downloads/vol_record")
...
for (f in fls_train){
  ...
  vol_record_train[[i]] <- temp
  i <- i + 1
}

i <- 1
vol_record_test <- list()
for (f in fls_test){
  path <- file.path(str_c(d, "/", f)) 
  temp <- read.csv(path)
  temp <- temp %>% select(-X)
  ...
  vol_record_test[[i]] <- temp
  i <- i + 1
}

## XGBoost
set.seed(234)
conf_vol_record_xgb <- list()
for (i in 1:10){
  train <- vol_record_train[[i]]
  test <- vol_record_test[[i]]
  ...
  conf_vol_record_xgb[[i]] <- conf_mat$table
}

## Random Forest
set.seed(234)
conf_vol_record_rf <- list()
for (i in 1:10){
  train <- vol_record_train[[i]]
  test <- vol_record_test[[i]]
  ...
  conf_vol_record_rf[[i]] <- conf_mat$table
  cat("Fold", i, "complete \n", sep = " ")
}

## DNN
set.seed(234)
conf_vol_record_dnn <- list()
for (i in 1:10){
  train <- vol_record_train[[i]]
  test <- vol_record_test[[i]]
  ...
  conf_vol_record_dnn[[i]] <- conf_mat$table
  cat("Fold", i, "complete \n", sep = " ")
}

## Confusion Matrix
confusion_matrix(conf_vol_record_xgb)
confusion_matrix(conf_vol_record_rf)
confusion_matrix(conf_vol_record_dnn)
