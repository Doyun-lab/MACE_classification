...
setwd("/Users/kwondoyun/Downloads/tiva_newest")
...
for (f in fls_test){
  ...
  tiva_new_test[[i]] <- temp
  i <- i + 1
}

## XGBoost
set.seed(234)
conf_tiva_new_xgb <- list()
for (i in 1:10){
  train <- tiva_new_train[[i]]
  test <- tiva_new_test[[i]]
  ...
  conf_tiva_new_xgb[[i]] <- conf_mat$table
}

## Random Forest
set.seed(234)
conf_tiva_new_rf <- list()
for (i in 1:10){
  train <- tiva_new_train[[i]]
  test <- tiva_new_test[[i]]
  ...
  conf_tiva_new_rf[[i]] <- conf_mat$table
  cat("Fold", i, "complete \n", sep = " ")
}

## DNN
set.seed(234)
conf_tiva_new_dnn <- list()
for (i in 1:10){
  train <- tiva_new_train[[i]]
  test <- tiva_new_test[[i]]
  ...
  conf_tiva_new_dnn[[i]] <- conf_mat$table
  cat("Fold", i, "complete \n", sep = " ")
}

## Confusion Matrix
confusion_matrix(conf_tiva_new_xgb)
confusion_matrix(conf_tiva_new_rf)
confusion_matrix(conf_tiva_new_dnn)
