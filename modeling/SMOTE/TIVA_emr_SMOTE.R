...
setwd("/Users/kwondoyun/Downloads/tiva_emr")
...
## XGBoost
set.seed(234)
conf_tiva_emr_xgb <- list()
for (i in 1:10){
  train <- tiva_emr_train[[i]]
  test <- tiva_emr_test[[i]]
  ...
  conf_tiva_emr_xgb[[i]] <- conf_mat$table
}

## Random Forest
set.seed(234)
conf_tiva_emr_rf <- list()
for (i in 1:10){
  train <- tiva_emr_train[[i]]
  test <- tiva_emr_test[[i]]
  ...
  conf_tiva_emr_rf[[i]] <- conf_mat$table
  cat("Fold", i, "complete", sep = " ")
}

## DNN
set.seed(234)
conf_tiva_emr_dnn <- list()
for (i in 1:10){
  train <- tiva_emr_train[[i]]
  test <- tiva_emr_test[[i]]
  ...
  conf_tiva_emr_dnn[[i]] <- conf_mat$table
  cat("Fold", i, "complete", sep = " ")
}

## Confusion Matrix
confusion_matrix(conf_tiva_emr_xgb)
confusion_matrix(conf_tiva_emr_rf)
confusion_matrix(conf_tiva_emr_dnn)
