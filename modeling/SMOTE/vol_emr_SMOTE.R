...
setwd("/Users/kwondoyun/Downloads/vol_emr")
...
## XGBoost
set.seed(234)
conf_vol_emr_xgb <- list()
for (i in 1:10){
  train <- vol_emr_train[[i]]
  test <- vol_emr_test[[i]]
  ...
  conf_vol_emr_xgb[[i]] <- conf_mat$table
}

## Random Forest
set.seed(234)
conf_vol_emr_rf <- list()
for (i in 1:10){
  train <- vol_emr_train[[i]]
  test <- vol_emr_test[[i]]
  ...
  conf_vol_emr_rf[[i]] <- conf_mat$table
  cat("Fold", i, "complete \n", sep = " ")
}

## DNN
set.seed(234)
conf_vol_emr_dnn <- list()
for (i in 1:10){
  train <- vol_emr_train[[i]]
  test <- vol_emr_test[[i]]
  ...
  conf_vol_emr_dnn[[i]] <- conf_mat$table
  cat("Fold", i, "complete \n", sep = " ")
}

## Confusion Matrix
confusion_matrix(conf_vol_emr_xgb)
confusion_matrix(conf_vol_emr_rf)
confusion_matrix(conf_vol_emr_dnn)
