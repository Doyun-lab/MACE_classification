...
setwd("/Users/kwondoyun/Downloads/tiva_record")
...
for (f in fls_train){
  ...
  tiva_record_train[[i]] <- temp
  i <- i + 1
}

i <- 1
tiva_record_test <- list()
for (f in fls_test){
  path <- file.path(str_c(d, "/", f)) 
  temp <- read.csv(path)
  temp <- temp %>% select(-X)
  ...
  tiva_record_test[[i]] <- temp
  i <- i + 1
}

## XGBoost
set.seed(234)
conf_tiva_record_xgb <- list()
for (i in 1:10){
  train <- tiva_record_train[[i]]
  test <- tiva_record_test[[i]]
  ...
  conf_tiva_record_xgb[[i]] <- conf_mat$table
}

## Random Forest
set.seed(234)
conf_tiva_record_rf <- list()
for (i in 1:10){
  train <- tiva_record_train[[i]]
  test <- tiva_record_test[[i]]
  ...
  conf_tiva_record_rf[[i]] <- conf_mat$table
  cat("Fold", i, "complete \n", sep = " ")
}

## DNN
set.seed(234)
conf_tiva_record_dnn <- list()
for (i in 1:10){
  train <- tiva_record_train[[i]]
  test <- tiva_record_test[[i]]
  ...
  conf_tiva_record_dnn[[i]] <- conf_mat$table
  cat("Fold", i, "complete \n", sep = " ")
}

## Confusion Matrix
confusion_matrix(conf_tiva_record_xgb)
confusion_matrix(conf_tiva_record_rf)
confusion_matrix(conf_tiva_record_dnn)
