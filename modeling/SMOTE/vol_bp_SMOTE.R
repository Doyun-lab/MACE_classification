...
setwd("/Users/kwondoyun/Downloads/vol_bp")
...
## XGBoost
set.seed(234)
conf_vol_bp_xgb <- list()
for (i in 1:10){
  train <- vol_bp_train[[i]]
  test <- vol_bp_test[[i]]
  ...
  conf_vol_bp_xgb[[i]] <- conf_mat$table
}
...
for (i in 1:10){
  ...
  conf_vol_bp_rf[[i]] <- conf_mat$table
  cat("Fold", i, "complete \n", sep = " ")
}

## DNN
set.seed(234)
conf_vol_bp_dnn <- list()
for (i in 1:10){
  train <- vol_bp_train[[i]]
  test <- vol_bp_test[[i]]
  ...
  conf_vol_bp_dnn[[i]] <- conf_mat$table
  cat("Fold", i, "complete \n", sep = " ")
}

## Confusion Matrix
confusion_matrix(conf_vol_bp_xgb)
confusion_matrix(conf_vol_bp_rf)
confusion_matrix(conf_vol_bp_dnn)
