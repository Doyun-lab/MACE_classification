...
result_kernel <- data.frame()
kernel.w <- c('linear', 'polynomial', 'radial', 'sigmoid')
for (i in kernel.w){
  svm.model=svm(train_x,y=NULL,
                type='one-classification',
                nu=0.10,
  ...
  total_result = data.frame(accuracy, sensi, specif, i)
  
  result_kernel = rbind(result_kernel, total_result)
}
