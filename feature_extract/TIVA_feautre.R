...
vital_TIVA_total = readRDS('preprocessing\\vital_TIVA_NA_END_0122.rds')
names(vital_TIVA_total)
length(unique(vital_TIVA_total))
summary(vital_TIVA_total)

# 변수형태 변환 
vital_TIVA_total$RR = as.numeric(vital_TIVA_total$RR)
vital_TIVA_total$TV = as.numeric(vital_TIVA_total$TV)
vital_TIVA_total$BIS.BIS = as.numeric(vital_TIVA_total$BIS.BIS)

# 특징 추출할 변수 선택 
col_except = c("Time","Case_ID","Hypertension","Hypotension")
col_ = names(vital_TIVA_total)[!names(vital_TIVA_total)%in%col_except]
col_
...
saveRDS(TIVA_summary,"TIVA_summary_0122.rds")
...
names(TIVA_cpt)[1:length(TIVA_cpt)-1] = paste0("cpt",names(TIVA_cpt)[1:length(TIVA_cpt)-1])
names(TIVA_cpt)
saveRDS(TIVA_cpt, "TIVA_cpt_0122.rds")
...
nrow(TIVA_cre)
names(TIVA_cre)[1:length(TIVA_cre)-1] = paste0("cre_",names(TIVA_cre)[1:length(TIVA_cre)-1])
names(TIVA_cre)
saveRDS(TIVA_cre, "TIVA_cre_0122.rds")
