...
vital_volatile_total = readRDS('preprocessing\\vital_volatile_NA_end_ext.rds')
...
same_name # End.stage.renal.disease.ESRD.

col_except = c("Time","Case_ID","Hypertension","Hypotension", "End.stage.renal.disease.ESRD.")
...
saveRDS(volatile_summary,"volatile_summary_ext.rds")
...
cpt_f = function(data){
  ...
  cpt_mean = cpts(cpt.mean(data,penalty="BIC",method = "PELT"))[1]
  cpt_var = cpts(cpt.var(data,penalty="BIC", method = "PELT"))[1]
  cpt_meanvar = cpts(cpt.meanvar(data,penalty="BIC",method = "PELT"))[1]
...
names(volatile_cpt)[1:length(volatile_cpt)-1] = paste0("cpt_pelt",names(volatile_cpt)[1:length(volatile_cpt)-1])
names(volatile_cpt)
saveRDS(volatile_cpt, "volatile_cpt_PELT_ext.rds")
...
saveRDS(volatile_peak, "volatile_peak_mpd10_ext.rds")
...
names(volatile_cre)
saveRDS(volatile_cre, "volatile_cre_ext.rds")
...
saveRDS(volatile_freq, "volatile_freq_ext.rds")
