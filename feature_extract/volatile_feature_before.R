...
table(emr_volatile$OP.Time, useNA = "ifany")  # 수술시간이 잘못된 것: 3개. 

vital_volatile_total = readRDS('preprocessing\\vital_TIVA_NA_END.rds')
names(vital_volatile_total)

emr_vital_tiva = vital_volatile_total%>%left_join(emr_volatile,by='Case_ID')
...
emr_volatile = emr_volatile[,!names(emr_volatile)%in%same_name]
...
col_ = names(vital_volatile_total)[c(1:3,7:9)]  # Time, Case_id를 제외하고 특징을 추출할 변수들 

data = vital_volatile_total[vital_volatile_total$Case_ID == unique(vital_volatile_total$Case_ID)[2],]
summary(data)
...
saveRDS(TIVA_summary,"preprocessing/volatile_summary.rds")
...
names(TIVA_cpt)
saveRDS(TIVA_cpt, "preprocessing/volatile_cpt.rds")
...
saveRDS(TIVA_peak, "preprocessing/volatile_peak.rds")
...
saveRDS(TIVA_cre, "preprocessing/volatile_cre.rds")
...
saveRDS(TIVA_freq, "preprocessing/volatile_freq.rds")